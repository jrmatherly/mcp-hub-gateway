...
func getGordonSetup(ctx context.Context) MCPClientCfg {
	result := MCPClientCfg{
		MCPClientCfgBase: MCPClientCfgBase{
			...
			Icon:        "https://raw.githubusercontent.com/jrmatherly/mcp-hub-gateway/main/img/client/gordon.png",
...
func connectGordon(ctx context.Context) error {
	return exec.CommandContext(ctx, "docker", "ai", "config", "set-feature", "MCP Catalog", "true").
		Run()
}

func disconnectGordon(ctx context.Context) error {
	return exec.CommandContext(ctx, "docker", "ai", "config", "set-feature", "MCP Catalog", "false").
		Run()
}
