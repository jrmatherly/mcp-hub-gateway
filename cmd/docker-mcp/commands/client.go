...
import (
	...
	"github.com/jrmatherly/mcp-hub-gateway/cmd/docker-mcp/client"
)
...
func clientCommand(cwd string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: fmt.Sprintf(
			"client (Supported: %s)",
			strings.Join(client.GetSupportedMCPClients(*cfg), ", "),
		),
...
func connectClientCommand(cwd string, cfg client.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: fmt.Sprintf(
			"connect [OPTIONS] <mcp-client>\n\nSupported clients: %s",
			strings.Join(client.GetSupportedMCPClients(cfg), " "),
		),
		Short: fmt.Sprintf(
			"Connect the Docker MCP Toolkit to a client. Supported clients: %s",
			strings.Join(client.GetSupportedMCPClients(cfg), " "),
		),
...
func disconnectClientCommand(cwd string, cfg client.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: fmt.Sprintf(
			"disconnect [OPTIONS] <mcp-client>\n\nSupported clients: %s",
			strings.Join(client.GetSupportedMCPClients(cfg), " "),
		),
		Short: fmt.Sprintf(
			"Disconnect the Docker MCP Toolkit from a client. Supported clients: %s",
			strings.Join(client.GetSupportedMCPClients(cfg), " "),
		),
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
