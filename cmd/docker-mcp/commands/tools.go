...
import (
	...
	"github.com/jrmatherly/mcp-hub-gateway/cmd/docker-mcp/tools"
)
...
func toolsCommand(docker docker.Client) *cobra.Command {
	...
	cmd.PersistentFlags().
		StringSliceVar(&gatewayArgs, "gateway-arg", nil, "Additional arguments passed to the gateway")
	...
	enableCmd.Flags().
		StringVar(&enableServerName, "server", "", "Specify which server provides the tools (optional, will auto-discover if not provided)")
	...
	disableCmd.Flags().
		StringVar(&disableServerName, "server", "", "Specify which server provides the tools (optional, will auto-discover if not provided)")
