...
func TestDockerCatalogProtection(t *testing.T) {
	...
	t.Run("TestCreateDockerCatalogPrevented", func(t *testing.T) {
		...
		assert.Contains(
			t,
			err.Error(),
			"cannot create catalog 'docker-mcp' as it is reserved for Docker's official catalog",
		)
	})
	...
	t.Run("TestRemoveDockerCatalogPrevented", func(t *testing.T) {
		...
		assert.Contains(
			t,
			err.Error(),
			"cannot remove catalog 'docker-mcp' as it is managed by Docker",
		)
	})
...
`
		...
		assert.Contains(
			t,
			err.Error(),
			"cannot add servers to catalog 'docker-mcp' as it is managed by Docker",
		)
	})
	...
	t.Run("TestExportDockerCatalogPrevented", func(t *testing.T) {
		...
		assert.Contains(
			t,
			err.Error(),
			"cannot export the Docker MCP catalog as it is managed by Docker",
		)
	})
