...
func TestConditionalConfiguredCatalogPaths(t *testing.T) {
	t.Run("excludes configured catalogs when single Docker catalog URL", func(t *testing.T) {
		...
		shouldExclude := len(defaultPaths) == 1 &&
			(defaultPaths[0] == "https://desktop.docker.com/mcp/catalog/v2/catalog.yaml" || defaultPaths[0] == "docker-mcp.yaml")
		assert.True(
			t,
			shouldExclude,
			"should exclude configured catalogs when single Docker catalog URL",
		)
	})
	...
	t.Run("excludes configured catalogs when single Docker catalog filename", func(t *testing.T) {
		...
		assert.True(
			t,
			shouldExclude,
			"should exclude configured catalogs when single Docker catalog filename",
		)
	})
	...
	t.Run("includes configured catalogs when single non-Docker catalog", func(t *testing.T) {
		...
		assert.False(
			t,
			shouldExclude,
			"should include configured catalogs when single non-Docker catalog",
		)
	})
}
