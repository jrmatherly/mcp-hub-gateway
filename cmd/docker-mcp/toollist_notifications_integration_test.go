...
func TestIntegrationToolListChangeNotifications(t *testing.T) {
	...
	transport := &mcp.CommandTransport{
		Command: exec.CommandContext(context.TODO(), "docker", args...),
	}
	...
	case <-notificationChan:
		...
		require.True(
			t,
			dynamicToolFound,
			"Dynamic tool 'dynamic_tool' should be visible in tool list after notification",
		)
		...
		if len(dynamicResponse.Content) > 0 {
			if textContent, ok := dynamicResponse.Content[0].(*mcp.TextContent); ok {
				require.Contains(
					t,
					textContent.Text,
					"Dynamic tool 'dynamic_tool' executed",
					"Dynamic tool should return expected response",
				)
		...
		t.Logf(
			"✅ SUCCESS: Tool list change notifications are received and RefreshCapabilities updates the tool list correctly",
		)
		t.Logf(
			"✅ SUCCESS: Dynamic tool support is fully functional - tools appear in ListTools and are callable",
		)

	case <-time.After(10 * time.Second):
		t.Fatal(
			"FAIL: Tool list change notification was not received within 10 seconds. This indicates the gateway is not properly forwarding tool list change notifications from MCP servers to clients.",
		)
	}
...
func TestIntegrationToolListNotificationRouting(t *testing.T) {
	...
	transport1 := &mcp.CommandTransport{
		Command: exec.CommandContext(context.TODO(), "docker", args...),
	}
	...
	case <-client1NotificationChan:
		t.Log(
			"Client 1 received notification successfully - gateway notification routing is working",
		)
		mu1.Lock()
	...
	case <-time.After(3 * time.Second):
		t.Log(
			"No notifications received by client 1 - this is expected if the MCP server doesn't trigger tool list changes",
		)
		...
		t.Logf(
			"Client 1 notification handler configured: %v",
			notificationState == false,
		) // false means handler exists but wasn't called
	}
