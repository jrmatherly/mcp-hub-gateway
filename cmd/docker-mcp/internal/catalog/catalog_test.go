...
func TestCatalogGetWithoutConfigured(t *testing.T) {
	...
	assert.NotContains(
		...
		catalog.Servers,
		"custom-server",
		"should NOT contain configured catalog server",
	)
}
...
func TestCatalogPrecedenceOrder(t *testing.T) {
	...
	assert.Equal(
		...
		"should use CLI catalog version (highest precedence)",
	)
	...
	assert.Equal(
		t,
		"overlapping-tool",
		overlappingServer.Tools[0].Name,
		"tool name should be preserved",
	)
	assert.Equal(
		t,
		"CLI Catalog Server",
		overlappingServer.Tools[0].Description,
		"should use entire server from highest precedence catalog",
	)
}
