	...
	t.Run("intercepts wrapped GitHub auth error", func(t *testing.T) {
		mockHandler := func(_ context.Context, _ string, _ mcp.Request) (mcp.Result, error) {
			return &mcp.CallToolResult{
				...
				Content: []mcp.Content{
					&mcp.TextContent{
						Text: `calling "tools/call": GitHub API error: 401 Bad credentials`,
					},
				},
			}, nil
		}
		...
		mockHandler := func(_ context.Context, _ string, _ mcp.Request) (mcp.Result, error) {
			return &mcp.CallToolResult{
				...
				Content: []mcp.Content{
					...
					&mcp.TextContent{
						Text: "failed to list repos: GET https://api.github.com/user/repos: 401 Bad credentials",
					},
				},
			}, nil
		}
