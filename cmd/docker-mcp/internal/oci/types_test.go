...
func TestServerDetailParsing(t *testing.T) {
	...
	if serverDetail.Name != "io.github.slimslenderslacks/garmin_mcp" {
		t.Errorf(
			"Expected name 'io.github.slimslenderslacks/garmin_mcp', got '%s'",
			serverDetail.Name,
		)
	}
...
func TestConversionForFileSystem(t *testing.T) {
	...
	if serverDetail.Name != "io.github.slimslenderslacks/filesystem" {
		t.Errorf(
			"Expected name 'io.github.slimslenderslacks/filesystem', got '%s'",
			serverDetail.Name,
		)
	}
	...
	if serverDetail.Repository.URL != "https://github.com/modelcontextprotocol/servers" {
		t.Errorf(
			"Expected repository URL 'https://github.com/modelcontextprotocol/servers', got '%s'",
	...
	if runtimeArg.Value != "type=bind,src={source_path},dst={target_path}" {
		t.Errorf(
			"Expected runtime argument value 'type=bind,src={source_path},dst={target_path}', got '%s'",
			runtimeArg.Value,
		)
	}
	...
	if packageArg.ValueHint != "target_dir" {
		t.Errorf(
			"Expected package argument value hint 'target_dir', got '%s'",
			packageArg.ValueHint,
		)
	}
	...
	if env.Description != "Logging level (debug, info, warn, error)" {
		t.Errorf(
			"Expected env var description 'Logging level (debug, info, warn, error)', got '%s'",
			env.Description,
		)
	}
	...
	if catalogServer.Description != "Node.js server implementing Model Context Protocol (MCP) for filesystem operations." {
		t.Errorf(
			"Expected catalog server description to match, got '%s'",
	...
	if catalogServer.Image != "mcp/filesystem:1.0.2" {
		t.Errorf(
			"Expected catalog server image 'mcp/filesystem:1.0.2', got '%s'",
			catalogServer.Image,
		)
	}
	...
	if len(catalogServer.Env) != len(expectedEnvVars) {
		t.Errorf(
			"Expected %d environment variables, got %d",
			len(expectedEnvVars),
			len(catalogServer.Env),
		)
	} else {
...
func TestBasicServerConversion(t *testing.T) {
	...
	if serverDetail.Description != "construct new tools out of existing images" {
		t.Errorf(
			"Expected description 'construct new tools out of existing images', got '%s'",
	...
	if serverDetail.Repository.URL != "https://github.com/slimslenderslacks/poci" {
		t.Errorf(
			"Expected repository URL 'https://github.com/slimslenderslacks/poci', got '%s'",
			serverDetail.Repository.URL,
		)
	}
	if serverDetail.Repository.Source != "github" {
	...
	if catalogServer.Image != "jimclark106/poci:latest" {
		t.Errorf(
			"Expected catalog server image 'jimclark106/poci:latest', got '%s'",
			catalogServer.Image,
		)
	}
...
func TestRemoteServerConversion(t *testing.T) {
	...
	if remote.URL != "http://mcp-fs.anonymous.modelcontextprotocol.io/sse" {
		t.Errorf(
			"Expected remote URL 'http://mcp-fs.anonymous.modelcontextprotocol.io/sse', got '%s'",
			remote.URL,
		)
	}
	...
	if apiKeyHeader.Description != "API key for authentication" {
		t.Errorf(
			"Expected first header description 'API key for authentication', got '%s'",
			apiKeyHeader.Description,
		)
	}
	...
	if regionHeader.Description != "Service region" {
		t.Errorf(
			"Expected second header description 'Service region', got '%s'",
			regionHeader.Description,
		)
	}
	if regionHeader.DefaultValue != "us-east-1" {
		t.Errorf(
			"Expected X-Region header default 'us-east-1', got '%v'",
			regionHeader.DefaultValue,
		)
	}
	...
	if catalogServer.Description != "remote example" {
		t.Errorf(
			"Expected catalog server description 'remote example', got '%s'",
			catalogServer.Description,
		)
	}
	...
	if catalogServer.Remote.URL != "http://mcp-fs.anonymous.modelcontextprotocol.io/sse" {
		t.Errorf(
			"Expected catalog remote URL 'http://mcp-fs.anonymous.modelcontextprotocol.io/sse', got '%s'",
			catalogServer.Remote.URL,
		)
	}
	if catalogServer.Remote.Transport != "sse" {
		t.Errorf(
			"Expected catalog remote transport 'sse', got '%s'",
			catalogServer.Remote.Transport,
		)
	}
	...
	if len(catalogServer.Remote.Headers) != len(expectedHeaders) {
		t.Errorf(
			...
			len(expectedHeaders),
			len(catalogServer.Remote.Headers),
		)
	}
