...
func (sd *ServerDetail) ToCatalogServer() catalog.Server {
	...
	if len(sd.Packages) > 0 {
		...
		for _, envVar := range pkg.Env {
			value, secrets, config := getKeyValueInput(
				envVar,
				CanonicalizeServerName(sd.Name),
				false,
			)
			if len(secrets) > 0 {
		...
		for _, arg := range pkg.PackageArguments {
			switch arg.Type {
			case "positional":
				value, secrets, configSchema := getInput(
					arg.InputWithVariables,
					CanonicalizeServerName(sd.Name),
				)
...
func getKeyValueInput(
	kvi KeyValueInput,
	serverName string,
	useEnvForm bool,
) (string, []catalog.Secret, map[string]any) {
	if len(kvi.Variables) == 0 {
		...
		return fmt.Sprintf(
			"{{%s}}",
			fmt.Sprintf("%s.%s", serverName, kvi.Name),
		), []catalog.Secret{}, config
	}
