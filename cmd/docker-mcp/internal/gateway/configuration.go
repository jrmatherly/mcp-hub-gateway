...
func (c *Configuration) Find(
	serverName string,
) (*catalog.ServerConfig, *map[string]catalog.Tool, bool) {
...
func (c *FileBasedConfiguration) Read(
	ctx context.Context,
) (Configuration, chan Configuration, func() error, error) {
...
func (c *FileBasedConfiguration) readOnce(ctx context.Context) (Configuration, error) {
	...
	for serverName, server := range ociServers {
		if _, exists := servers[serverName]; exists {
			log(
				fmt.Sprintf(
					"Warning: server '%s' from OCI reference overwrites server from catalog",
					serverName,
				),
			)
		}
	...
	if len(c.MCPRegistryServers) > 0 {
		for i, mcpServer := range c.MCPRegistryServers {
			...
			log(
				fmt.Sprintf(
					"Added MCP registry server: %s (image: %s)",
					serverName,
					mcpServer.Image,
				),
			)
	...
	for _, registryPath := range c.RegistryPath {
		...
		for serverName, tile := range cfg.Servers {
			if _, exists := mergedRegistry.Servers[serverName]; exists {
				log(
					fmt.Sprintf(
						"Warning: overlapping server '%s' found in registry '%s', overwriting previous value",
						serverName,
						registryPath,
					),
				)
...
func (c *FileBasedConfiguration) readConfig(
	ctx context.Context,
	...
	for _, configPath := range c.ConfigPath {
		...
		for serverName, serverConfig := range cfg {
			if _, exists := mergedConfig[serverName]; exists {
				log(
					fmt.Sprintf(
						"Warning: overlapping server config '%s' found in config file '%s', overwriting previous value",
						serverName,
						configPath,
					),
				)
	...
	for _, toolsPath := range c.ToolsPath {
		...
		for serverName, serverTools := range toolsConfig.ServerTools {
			if _, exists := mergedToolsConfig.ServerTools[serverName]; exists {
				log(
					fmt.Sprintf(
						"Warning: overlapping server tools '%s' found in tools file '%s', overwriting previous value",
						serverName,
						toolsPath,
					),
				)
...
func (c *FileBasedConfiguration) readDockerDesktopSecrets(
	ctx context.Context,
	servers map[string]catalog.Server,
	serverNames []string,
...
func (c *FileBasedConfiguration) readSecretsFromFile(
	ctx context.Context,
	path string,
) (map[string]string, error) {
	secrets := map[string]string{}
...
func (c *FileBasedConfiguration) readServersFromOci(
	_ context.Context,
	...
	for _, ociRef := range c.OciRef {
		...
		for i, ociServer := range ociCatalog.Registry {
			...
			if _, exists := ociServers[serverName]; exists {
				log(
					fmt.Sprintf(
						"Warning: overlapping server '%s' found in OCI reference '%s', overwriting previous value",
						serverName,
						ociRef,
					),
				)
