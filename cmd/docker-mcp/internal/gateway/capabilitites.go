...
func (g *Gateway) listCapabilities(
	ctx context.Context,
	...
	for _, serverName := range serverNames {
		...
		case serverConfig != nil:
			errs.Go(func() error {
				...
				resourceTemplates, err := client.Session().
					ListResourceTemplates(ctx, &mcp.ListResourceTemplatesParams{})
				if err == nil {
					// Record the number of resource templates discovered from this server
					telemetry.RecordResourceTemplateList(
						ctx,
						serverConfig.Name,
					...
					for _, resourceTemplate := range resourceTemplates.ResourceTemplates {
						capabilities.ResourceTemplates = append(
							capabilities.ResourceTemplates,
							ResourceTemplateRegistration{
								ResourceTemplate: *resourceTemplate,
								Handler: g.mcpServerResourceHandler(
									serverConfig,
									g.mcpServer,
								),
							},
				...
				if len(capabilities.ResourceTemplates) > 0 {
					log += fmt.Sprintf(
						" (%d resourceTemplates)",
						len(capabilities.ResourceTemplates),
					)
				}
				if log != "" {
...
func isToolEnabled(
	configuration Configuration,
	serverName, serverImage, toolName string,
	enabledTools []string,
) bool {
