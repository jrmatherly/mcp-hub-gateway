...
import (
	...
	"github.com/jrmatherly/mcp-hub-gateway/cmd/docker-mcp/internal/sliceutil"
...
func RunNetworkProxies(
	...
	cli docker.Client,
	proxies []Proxy,
	keepCtrs, debugDNS bool,
) (_ TargetConfig, _ func(context.Context) error, retErr error) {
	if len(proxies) == 0 {
...
func removeProxies(
	...
	logf(
		"    > Removing proxies (%s) and networks (%s)",
		strings.Join(proxyNames, ", "),
		strings.Join(nwNames, ", "),
	)
