...
import (
	...
	"github.com/jrmatherly/mcp-hub-gateway/cmd/docker-mcp/internal/eval"
	"github.com/jrmatherly/mcp-hub-gateway/cmd/docker-mcp/internal/gateway/proxies"
	mcpclient "github.com/jrmatherly/mcp-hub-gateway/cmd/docker-mcp/internal/mcp"
)
...
func (cp *clientPool) longLived(serverConfig *catalog.ServerConfig, config *clientConfig) bool {
	keep := config != nil && config.serverSession != nil &&
		(serverConfig.Spec.LongLived || cp.LongLived)
	return keep
}

func (cp *clientPool) AcquireClient(
	ctx context.Context,
...
func (cp *clientPool) runToolContainer(
	ctx context.Context,
	tool catalog.Tool,
...
func (cp *clientPool) argsAndEnv(
	serverConfig *catalog.ServerConfig,
	readOnly *bool,
	targetConfig proxies.TargetConfig,
) ([]string, []string) {
