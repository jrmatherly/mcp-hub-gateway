...
func (c *dockerClient) ContainerExists(
	ctx context.Context,
	container string,
) (bool, container.InspectResponse, error) {
...
func (c *dockerClient) StartContainer(
	...
	resp, err := c.apiClient().
		ContainerCreate(ctx, &containerConfig, &hostConfig, &networkingConfig, nil, containerID)
	if err != nil {
...
func (c *dockerClient) InspectContainer(
	ctx context.Context,
	containerID string,
...
func (c *dockerClient) FindAllContainersByLabel(
	ctx context.Context,
	label string,
) ([]string, error) {
	containers, err := c.apiClient().ContainerList(ctx, container.ListOptions{
