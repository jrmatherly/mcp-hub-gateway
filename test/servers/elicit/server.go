		...
		func(_ context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			return &mcp.CallToolResult{
				Content: []mcp.Content{
					&mcp.TextContent{
						Text: fmt.Sprintf(
							"elicit result: action=%s, content=%+v",
							result.Action,
							result.Content,
						),
					},
	...
	server.AddTool(
		...
		func(_ context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if !ok {
				return &mcp.CallToolResult{
					Content: []mcp.Content{
						&mcp.TextContent{Text: "action parameter must be a string"},
					},
				}, nil
			}
			...
			if !ok {
				return &mcp.CallToolResult{
					Content: []mcp.Content{
						&mcp.TextContent{Text: "toolName parameter must be a string"},
					},
				}, nil
			}
			...
			case "add":
				...
				server.AddTool(
					...
					func(_ context.Context, _ *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
						return &mcp.CallToolResult{
							Content: []mcp.Content{
								&mcp.TextContent{
									Text: fmt.Sprintf("Dynamic tool '%s' executed", toolName),
								},
							},
						}, nil
					},
				...
				return &mcp.CallToolResult{
					Content: []mcp.Content{
						&mcp.TextContent{Text: fmt.Sprintf("Added tool '%s'", toolName)},
					},
				}, nil
			...
			case "remove":
				...
				return &mcp.CallToolResult{
					Content: []mcp.Content{
						&mcp.TextContent{Text: fmt.Sprintf("Removed tool '%s'", toolName)},
					},
				}, nil
			...
			default:
				return &mcp.CallToolResult{
					Content: []mcp.Content{
						&mcp.TextContent{
							Text: fmt.Sprintf("Unknown action '%s'. Use 'add' or 'remove'", action),
						},
					},
				}, nil
			}
